...
node* find(node* n, node* head, int val) {
	if (head->value == val) {
		return head;
	}
	...
	if (n->left != NULL && val < n->value) {
		return find(n->left, head, val);
	}
	if (n->right != NULL && val > n->value) {
		return find(n->right, head, val);
	}
}
